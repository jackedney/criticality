...
export function findTodoFunctions(project: Project): TodoFunction[] {
  ...
  for (const sourceFile of project.getSourceFiles()) {
    ...
    if (!sourceFileText.includes('TODO') && !TODO_MACRO_REGEX.test(sourceFileText)) {
      continue;
    }
    ...
    for (const func of functions) {
      // Optimization: Extract body text from already loaded sourceFileText to avoid internal getText() calls
      ...
      const bodyText = sourceFileText.substring(body.getStart(), body.getEnd());
...
export function inspectAst(
  ...
  for (const func of functions) {
    ...
    const bodyText = body
      ? sourceFileText.substring(body.getStart(), body.getEnd())
      : undefined;
