/**
 * Tests for Protocol State Types (v2) - 3-tier type system.
 */
...
describe('Protocol State Types (v2)', () => {
  ...
  describe('Tier 2: PhaseState factory functions', () => {
    ...
    it('type guards correctly identify PhaseState variants', () => {
      ...
      expect(isCompositionAuditPhaseState(ignition)).toBe(false);
      expect(isInjectionPhaseState(ignition)).toBe(false);
      expect(isMesoscopicPhaseState(ignition)).toBe(false);
      expect(isMassDefectPhaseState(ignition)).toBe(false);

      expect(isIgnitionPhaseState(lattice)).toBe(false);
      expect(isLatticePhaseState(lattice)).toBe(true);
      expect(isCompositionAuditPhaseState(lattice)).toBe(false);
      expect(isInjectionPhaseState(lattice)).toBe(false);
      expect(isMesoscopicPhaseState(lattice)).toBe(false);
      expect(isMassDefectPhaseState(lattice)).toBe(false);

      expect(isIgnitionPhaseState(composition)).toBe(false);
      expect(isLatticePhaseState(composition)).toBe(false);
      expect(isCompositionAuditPhaseState(composition)).toBe(true);
      expect(isInjectionPhaseState(composition)).toBe(false);
      expect(isMesoscopicPhaseState(composition)).toBe(false);
      expect(isMassDefectPhaseState(composition)).toBe(false);

      expect(isIgnitionPhaseState(injection)).toBe(false);
      expect(isLatticePhaseState(injection)).toBe(false);
      expect(isCompositionAuditPhaseState(injection)).toBe(false);
      expect(isInjectionPhaseState(injection)).toBe(true);
      expect(isMesoscopicPhaseState(injection)).toBe(false);
      expect(isMassDefectPhaseState(injection)).toBe(false);

      expect(isIgnitionPhaseState(mesoscopic)).toBe(false);
      expect(isLatticePhaseState(mesoscopic)).toBe(false);
      expect(isCompositionAuditPhaseState(mesoscopic)).toBe(false);
      expect(isInjectionPhaseState(mesoscopic)).toBe(false);
      expect(isMesoscopicPhaseState(mesoscopic)).toBe(true);
      expect(isMassDefectPhaseState(mesoscopic)).toBe(false);

      expect(isIgnitionPhaseState(massDefect)).toBe(false);
      expect(isLatticePhaseState(massDefect)).toBe(false);
      expect(isCompositionAuditPhaseState(massDefect)).toBe(false);
      expect(isInjectionPhaseState(massDefect)).toBe(false);
      expect(isMesoscopicPhaseState(massDefect)).toBe(false);
  ...
  describe('Tier 1: BlockedState', () => {
    ...
    it('creates BlockedState with options', () => {
      const blockedState = createBlockedState({
      ...
      expect(blockedState.kind).toBe('Blocked');
      expect(blockedState.options).toEqual(['A', 'B', 'C']);
      expect(blockedState.timeoutMs).toBeUndefined();
    });

    it('creates BlockedState with timeoutMs', () => {
      const blockedState = createBlockedState({
      ...
      expect(blockedState.kind).toBe('Blocked');
      expect(blockedState.options).toBeUndefined();
      expect(blockedState.timeoutMs).toBe(300000);
    });

    it('creates BlockedState with both options and timeoutMs', () => {
      const blockedState = createBlockedState({
  ...
  describe('Tier 1: FailedState', () => {
    ...
    it('creates FailedState with code', () => {
      const failedState = createFailedState({
        phase: 'Lattice',
      ...
      expect(failedState.kind).toBe('Failed');
      expect(failedState.code).toBe('COMPILATION_ERROR');
      expect(failedState.context).toBeUndefined();
    });

    it('creates FailedState with recoverable set to true', () => {
      const failedState = createFailedState({
        phase: 'Injection',
        error: 'Timeout',
        recoverable: true,
    ...
    it('creates FailedState with context', () => {
      const failedState = createFailedState({
        phase: 'Mesoscopic',
        error: 'Test failed',
      ...
      expect(failedState.kind).toBe('Failed');
      expect(failedState.context).toBe('Stack trace here');
    });
  ...
  describe('ProtocolState union', () => {
    it('includes all Tier 1 state types', () => {
      const activeState: ProtocolState = createActiveState(
        createIgnitionPhaseState(createIgnitionInterviewing('Discovery', 0))
      );
      const blockedState: ProtocolState = createBlockedState({
        reason: 'user_requested',
        phase: 'Ignition',
        query: 'Approve?',
      });
      const failedState: ProtocolState = createFailedState({
        phase: 'Ignition',
        error: 'Error',
      });
      const completeState: ProtocolState = createCompleteState(['spec']);
      ...
      for (const state of states) {
        switch (state.kind) {
          ...
          case 'Failed':
            ...
            expect(state.recoverable).toBeDefined();
            break;
          case 'Complete':
  ...
  describe('Helper functions', () => {
    describe('getPhase', () => {
      ...
      it('works for all phases in ActiveState', () => {
        const phases = [
          createActiveState(createIgnitionPhaseState(createIgnitionInterviewing('Discovery', 0))),
        ...
        phases.forEach((phase, i) => {
          ...
          expect(getPhase(phase)).toBe(expectedPhases[i]);
        });
      });
    });
    ...
    describe('getStep', () => {
      ...
      it('extracts step from all ActiveState phases', () => {
        const states = [
          createActiveState(createIgnitionPhaseState(createIgnitionInterviewing('Discovery', 0))),
          createActiveState(createLatticePhaseState(createLatticeCompilingCheck(1))),
          createActiveState(createCompositionAuditPhaseState(createCompositionAuditAuditing(3))),
        ...
        states.forEach((state, i) => {
          expect(getStep(state)).toBe(expectedSteps[i]);
        });
      });

      it('returns undefined for BlockedState', () => {
        const state = createBlockedState({
          reason: 'user_requested',
          phase: 'Ignition',
          query: 'Approve?',
      ...
      it('returns undefined for FailedState', () => {
        const state = createFailedState({
          phase: 'Ignition',
          error: 'Error',
        });
    ...
    describe('isTerminalState', () => {
      it('returns true for CompleteState', () => {
        const state = createCompleteState(['spec']);
        expect(isTerminalState(state)).toBe(true);
      });

      it('returns true for FailedState', () => {
        const state = createFailedState({
          phase: 'Ignition',
          error: 'Error',
        });
      ...
      it('returns false for ActiveState', () => {
        const state = createActiveState(
          createIgnitionPhaseState(createIgnitionInterviewing('Discovery', 0))
        );
      ...
      it('returns false for BlockedState', () => {
        const state = createBlockedState({
          reason: 'user_requested',
          phase: 'Ignition',
          query: 'Approve?',
    ...
    describe('canTransition', () => {
      it('returns true for ActiveState in non-Complete phase', () => {
        const state = createActiveState(
          createIgnitionPhaseState(createIgnitionInterviewing('Discovery', 0))
        );
      ...
      it('returns false for FailedState', () => {
        const state = createFailedState({
          phase: 'Ignition',
          error: 'Error',
        });
      ...
      it('returns false for CompleteState', () => {
        const state = createCompleteState(['spec']);
        expect(canTransition(state)).toBe(false);
      });
    });
  ...
  describe('Readonly enforcement (compile-time)', () => {
    it('ActiveState properties are readonly at compile time', () => {
      ...
      // This should not compile - TypeScript prevents assignment to readonly properties
  ...
  describe('Edge cases', () => {
    ...
    it('creates BlockedState for all BlockReason variants', () => {
      const reasons: BlockReason[] = [
        'canonical_conflict',
        'unresolved_contradiction',
        'circuit_breaker',
      ...
      const states = reasons.map((reason) =>
        createBlockedState({
          reason,
          phase: 'Ignition',
          query: 'Approve?',
      ...
      expect(states.every(isBlockedState)).toBe(true);
      expect(states.map((s) => s.reason)).toEqual(reasons);
    });
    ...
    it('handles zero values correctly in optional fields', () => {
      const substate = createIgnitionSynthesizing(0);
      expect(substate.progress).toBe(0);

      const substate2 = createIgnitionInterviewing('Discovery', 0);
      expect(substate2.questionIndex).toBe(0);

      const substate3 = createLatticeCompilingCheck(0);
      expect(substate3.attempt).toBe(0);
    });

    it('handles negative values correctly in numeric fields', () => {
      const substate = createIgnitionInterviewing('Discovery', -1);
      expect(substate.questionIndex).toBe(-1);

      const substate2 = createMesoscopicExecutingCluster('c1', -0.5);
      expect(substate2.progress).toBe(-0.5);
    });

    it('handles empty arrays correctly', () => {
      const substate = createLatticeRepairingStructure([], 1);
      expect(substate.errors).toEqual([]);
    });

    it('handles single-element arrays correctly', () => {
      const substate = createLatticeRepairingStructure(['error1'], 1);
      expect(substate.errors).toEqual(['error1']);
    });

    it('creates CompleteState with empty artifacts array', () => {
      const state = createCompleteState([]);
      expect(state.artifacts).toEqual([]);
    });

    it('creates CompleteState with single artifact', () => {
      const state = createCompleteState(['spec']);
      expect(state.artifacts).toEqual(['spec']);
    });

    it('creates CompleteState with many artifacts', () => {
      const artifacts = ['spec', 'code', 'tests', 'witnesses', 'contracts', 'latticeCode'];
      const state = createCompleteState(artifacts);
  ...
  describe('Example: createActiveState with Injection implementing substate', () => {
    it('creates valid ActiveState with Injection implementing substate', () => {
      ...
      expect(activeState.phase.substate.step).toBe('implementing');
      // Cast to access implementing-specific properties
