...
describe('ClusterVerdict', () => {
  ...
  describe('handleClusterVerdict', () => {
    ...
    it('should return pass verdict when all claims passed', async () => {
      ...
      const result = await handleClusterVerdict({
        projectPath: '/tmp/test-cluster-verdict',
        cluster,
        claimResults,
        logger,
      });
    ...
    it('should trigger fallback when no CLAIM_REF links exist', async () => {
      ...
      expect(result.recordedClaims).toEqual(['balance_001', 'balance_002', 'balance_003']);
    ...
    it('should return fail verdict with violated claims', async () => {
      const claimResults: ClaimResult[] = [
        {
          claimId: 'balance_001',
          status: 'passed',
    ...
    it('should trigger fallback when no CLAIM_REF links exist', async () => {
      ...
      expect(result.verdict.functionsToReinject).toEqual([]);
      expect(result.verdict.fallbackTriggered).toBe(true);
      expect(result.recordedClaims).toEqual(['balance_001', 'balance_002']);
      expect(logger).toHaveBeenCalledWith(
        expect.stringContaining('No CLAIM_REF links found - triggering fallback')
      );
    });
  ...
  describe('processClusterVerdict', () => {
    it('should process pass verdict and return no recorded claims', async () => {
      const claimResults: ClaimResult[] = [
        {
          claimId: 'balance_001',
          status: 'passed',
    ...
    it('should process fail verdict and record violated claims', async () => {
      const claimResults: ClaimResult[] = [
        {
          claimId: 'balance_001',
          status: 'passed',
