...
export interface FunctionClaimEntry {
  readonly filePath: string;
  readonly claimRefs: readonly string[];
}

export type FunctionClaimMapping = Map<string, FunctionClaimEntry>;
...
/**
 * Builds a mapping of function names to their CLAIM_REFs with file paths.
 *
 * Scans all source files in the project and extracts
 * CLAIM_REF comments from function JSDocs, storing the
 * actual source file path for each function.
 *
 * @param project - The ts-morph Project.
 * @param projectPath - The project root path.
 * @returns Map of function name to entry containing filePath and claim references.
 */
function buildFunctionClaimMapping(project: Project, projectPath: string): FunctionClaimMapping {
  const mapping: FunctionClaimMapping = new Map();
  ...
  for (const sourceFile of project.getSourceFiles()) {
    ...
    let contracts;
    try {
      contracts = parseContracts(project, filePath);
    } catch (_error) {
      continue;
    }
    ...
    for (const contract of contracts) {
      if (contract.claimRefs.length > 0) {
        ...
        const existing = mapping.get(functionName);

        if (existing === undefined) {
          mapping.set(functionName, {
            filePath,
            claimRefs: [...contract.claimRefs],
          });
        } else {
          const allRefs = new Set([...existing.claimRefs, ...contract.claimRefs]);
          mapping.set(functionName, {
            filePath,
            claimRefs: Array.from(allRefs),
          });
        }
      }
    }
 ...
 * @param functionClaimMapping - Map of function name to entry containing filePath and claim references.
...
function identifyFunctionsToReinject(
  ...
  for (const [functionName, entry] of functionClaimMapping.entries()) {
    const violatedRefs = entry.claimRefs.filter((claimId) => violatedClaimsSet.has(claimId));
 ...
 * 2. Builds function -> claim mapping from CLAIM_REF comments with file paths
 * 3. Identifies functions that need re-injection using stored file paths
 * 4. Returns verdict with functions to re-inject
 * 5. Only records violated claims (fallback does not mark passed claims as violations)
 *
