 ...
 *
 * Each data model and interface in spec becomes a potential module.
    ...
    const relatedDataModels: string[] = iface.methods
      .flatMap((method) => {
        ...
        const allTypeNames = [...params, returnType].flatMap((paramOrReturn) => {
          /* eslint-disable-next-line security/detect-unsafe-regex -- safe: simple regex for type extraction */
 ...
 * claim relates to based on type references and subject.
 *
 * @param spec - The parsed specification.
...
/**
 * Finds modules that share claims with given module.
 *
