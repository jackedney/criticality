...
export async function serveRelay(options: RelayOptions = {}): Promise<RelayServer> {
  ...
  app.post('/pages', async (c) => {
    ...
    try {
      ...
      let foundTarget: { sessionId: string; target: ConnectedTarget } | undefined;

      while (Date.now() - startTime < TIMEOUT) {
        for (const [sessionId, target] of connectedTargets) {
          if (target.targetId === result.targetId) {
            foundTarget = { sessionId, target };
            break;
          }
        }
      ...
      namedPages.set(name, foundTarget.sessionId);
      await sendToExtension({
        method: 'forwardCDPCommand',
        params: {
          method: 'Target.activateTarget',
          params: { targetId: foundTarget.target.targetId },
        },
      });
      return c.json({
        wsEndpoint: `ws://${host}:${port}/cdp`,
        name,
        targetId: foundTarget.target.targetId,
        url: foundTarget.target.targetInfo.url,
      });
    } catch (err) {
