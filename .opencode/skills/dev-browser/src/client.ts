  ...
  return {
    ...
    async getAISnapshot(name: string): Promise<string> {
      ...
      const hasFunction = await page.evaluate(() => {
        return typeof (window as any).__devBrowser_getAISnapshot === 'function';
      });

      // Inject the snapshot script if not already present
      if (!hasFunction) {
      ...
      const snapshot = await page.evaluate(() => {
        return (window as any).__devBrowser_getAISnapshot();
      });
