...
function isChromeOrPlaywrightProcess(pid: string): boolean {
  try {
    const psOutput = execSync(`ps -p ${pid} -o comm=`, { encoding: 'utf-8' }).trim();
    const psArgs = execSync(`ps -p ${pid} -o args=`, { encoding: 'utf-8' }).trim();

    const knownBinaries = ['chrome', 'chromium', 'chrome-wrapper', 'headless-chrome', 'playwright'];

    const commMatch = knownBinaries.some((binary) => psOutput.includes(binary));
    const argsMatch = knownBinaries.some((binary) => psArgs.includes(binary));
    const remoteDebuggingMatch = psArgs.includes('--remote-debugging-port');

    return commMatch || argsMatch || remoteDebuggingMatch;
  } catch {
    return false;
  }
}
...
try {
  ...
  if (pid) {
    if (isChromeOrPlaywrightProcess(pid)) {
      console.log(`Cleaning up stale Chrome process on CDP port 9223 (PID: ${pid})`);
      execSync(`kill -9 ${pid}`);
    } else {
      const psOutput = execSync(`ps -p ${pid} -o comm=`, { encoding: 'utf-8' }).trim();
      console.warn(
        `Warning: Skipping kill - process on port 9223 (PID: ${pid}) is not Chrome/Playwright (command: ${psOutput})`
      );
    }
  }
} catch {
